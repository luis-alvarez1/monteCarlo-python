...
! first executable statement ............	
	...
	dg2rad=pi/180.D0
	vrsn='2.0.06b'; bldate='14 Sep 2009'
	write (*,*) ''
...
! *****************************************************************
!  write output grid files (File extension changed from ".asc" to ".txt", 7/29/3008, RLB)
  101	continue
	...
	do j=1,nout
	  ...
	  if (outp(4)) then ! depth of minimum factor of safety
	    ...
	    outfil=trim(folder)//trim(zfminfil)//trim(suffix)//'_'//trim(stp)//'.txt'
 	  ...
 	  if (outp(5)) then ! pressure head at depth of minimum factor of safety
	    ...
	    do i=1,imx1
 	    ...
 	    outfil=trim(folder)//trim(pminfil)//trim(suffix)//'_'//trim(stp)//'.txt'
	...
	if (outp(7) .and. unsat0) then ! incremental basal flux, unsaturated zone
	    do j=1,nts
	      ...
	      irfil='TRunszfluxTS'//trim(scratch)//trim(suffix)//'.txt'
 	...
 	if (ncc>0) then ! non-convergent cells, unsaturated zone
 	  outfil=trim(folder)//ncvfil//'UZ_'//trim(suffix)//'.txt'
 	...
 	if (nccs>0) then ! non-convergent cells, saturated zone
 	  outfil=trim(folder)//ncvfil//'SZ_'//trim(suffix)//'.txt'
